...
STEP_SIZE       = 30
GAMMA           = 0.5
CHECKPOINT_ROOT = "./CNN_sparse_checkpoints"  # <---------------------- Change this to path where to save the checkpoints
os.makedirs(CHECKPOINT_ROOT, exist_ok=True)

# Add sparsity configuration
MIN_SPARSITY = 0.0  # Minimum sparsity level (0% pruning)
...
for run_id in range(NUM_RUNS):
    ...
    print(f"\n=== Starting run {run_id+1}/{NUM_RUNS} | flip_p={flip_p:.2f}, pad={pad}"
          + (f", jitter" if jitter else "") + f", sparsity={sparsity:.2f} ===")
    ...
    for epoch in range(1, MAX_EPOCHS+1):
        ...
        # Apply sparsity before training
        apply_sparsity(model, sparsity)
        ...
        if val_acc > SAVE_THRESHOLD and saved_count < SAVE_TARGET:
            ckpt_path = os.path.join(
                run_folder,
                f"4_run{run_id}_save{saved_count+1:03d}_epoch{epoch:04d}_seed{seed:02d}.pth"
            )
            torch.save({
                'epoch': epoch,
