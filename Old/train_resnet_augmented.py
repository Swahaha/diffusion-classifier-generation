...
import os
CHECKPOINT_FOLDER = '/home/users/ren11/project/diffusion-data-aug/resnet_checkpoint'

def get_subset_indices(dataset, images_per_class=200):
    """Get indices for a subset of the dataset with equal number of images per class"""
    indices = []
...
VAL_BATCH_SIZE = 100
IMAGES_PER_CLASS = 200  # Number of images to use per class
...
subset_indices = get_subset_indices(train_set, IMAGES_PER_CLASS)
...
print(f"Using {len(subset_indices)} images for training ({IMAGES_PER_CLASS} per class)")
...
def train_val(model, criterion, optimizer, train_loader, val_loader, device, EPOCHS=100, INITIAL_LR=0.01, STEP_SIZE=30, GAMMA=0.1):
    ...
    for i in range(0, EPOCHS):
        ...
        if avg_acc > best_val_acc:
            ...
            save_path = os.path.join(CHECKPOINT_FOLDER, 'resnet_augmented_epoch_{}.pth'.format(i))
